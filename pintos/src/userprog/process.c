...
process_execute (const char *file_name) 
{
  ...
  /* Get the file name from the cmd_line (file_name) and store it in token */
  f_copy = malloc(strlen(file_name) + 1);
  ...
  while (elem != list_end(&(cur_thread->children))) {
    ...
    if (cur_child_wait_status->tid == tid)
      if (cur_child_wait_status->exit_code == -1) {
        list_remove(elem);
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  while (elem != list_end(&(cur_thread->children)) && !found ) {
    cur_child_wait_status = list_entry(elem,struct wait_status, child);
    if (cur_child_wait_status -> tid == child_tid) {
      found = true;
    }
    elem = list_next(elem);
  }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Getting file name from file_name */
  char *f_copy, *token, *save_ptr;
  f_copy = malloc(strlen(file_name) + 1);
  strlcpy (f_copy, file_name, strlen(file_name) + 1);
  token = strtok_r (f_copy, " ", &save_ptr);
...
static bool pass_arg (void **esp, const char *file_name) {
  int argc = 0;
  void *stack_ptr = *esp; 
  char *save_ptr, *save_ptr_counter;
  char *file = malloc (strlen (file_name) + 1);
  strlcpy (file, file_name, strlen(file_name) + 1);

  /* Count the number of tokens in file_name */
  int length = 0;
  char *tk_length = strtok_r (file, " ", &save_ptr_counter);
  while (tk_length != NULL) {
    length++;
    tk_length = strtok_r (NULL, " ", &save_ptr_counter);
  }
  free (file);
  ...
  strlcpy (file, file_name, strlen(file_name) + 1);
  char *token_list[length];
  char *token_l, *save_ptr_l;
  int index = 0;

  for (token_l = strtok_r (file, " ", &save_ptr_l); token_l != NULL;
        token_l = strtok_r (NULL, " ", &save_ptr_l)) {
    token_list[index] = malloc (strlen(token_l) + 1);
    strlcpy (token_list[index], token_l, strlen(token_l) + 1);
    index++;
  }
  free (file);

  /* Pushing arguments to stack and incrementing argc and store the pointers to arguments
     in arg_ptr_list */
  void *arg_ptr_list[length];
  size_t totalSize = 0; 
  for (index = length - 1; index >= 0; index--) {
    size_t tokensize = strlen (token_list[index]) + 1; // Need to take into account null terminator
    // Check that the stack page is not overflowed
    if (totalSize + tokensize > PGSIZE) {
      break;
    }
    stack_ptr = (void *) (((char *) stack_ptr) - tokensize);
    memcpy (stack_ptr, token_list[index], tokensize);
    free (token_list[index]);
    arg_ptr_list[argc] = stack_ptr;
    totalSize += tokensize;
    argc++;
  }

  /* Word-align the stack pointer to a multiple of 4 (New implementation) */
  uintptr_t wordAlign = (uintptr_t) stack_ptr;
  if (wordAlign % 4 != 0)
    wordAlign -= wordAlign % 4;
  stack_ptr = (void *) wordAlign;

  /* Push a null pointer sentinel */
  stack_ptr = (void *) (((char **)stack_ptr) - 1);
  *((char **)stack_ptr) = 0;
  ...
  *((char ***) stack_ptr) = argv;
  ...
  stack_ptr = (void *) ((int *) stack_ptr - 1);
  *((int *) stack_ptr) = argc;
